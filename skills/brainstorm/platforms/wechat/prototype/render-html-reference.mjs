...
function resolveMountedUrls(source, assetsDir, profileDir) {
  return source
    .replace(/(["'(=])\/assets\//g, `$1file://${assetsDir}/`)
    .replace(/(["'(=])\/profile\//g, `$1file://${profileDir}/`);
}
...
function main() {
  ...
  // Only rewrite URLs at their start. A blanket `/assets/` replacement also
  // matches the nested directory in `/profile/design-system/assets/...` and
  // corrupts every profile-owned image URL.
  fragment = resolveMountedUrls(fragment, assetsDir, profileDir);

  const css = resolveMountedUrls([
    path.join(profileDir, 'design-system', 'tokens.css'),
    path.join(profileDir, 'design-system', 'components.css'),
  ].map((f) => fs.readFileSync(f, 'utf8')).join('\n'), assetsDir, profileDir);
